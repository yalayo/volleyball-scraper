...
(defn create [{:keys [request _env]}]
  (js-await [data (cf/request->edn request)]
            (js-await [{:keys [success]} (db/run+ {:insert-into :props_leagues
...
(defn update [{:keys [route request _env]}]
  (let [id (js/parseInt (-> route :path-params :id) 10)]
    (js-await [data (cf/request->edn request)]
              (js-await [{:keys [success]} (db/run+ {:update :props_leagues
                                                      :set    {:name        (:name data)
...
(defn delete [{:keys [route _request _env]}]
  (let [id (js/parseInt (-> route :path-params :id) 10)]
    (js-await [{:keys [success]} (db/run+ {:delete-from :props_leagues
                                            :where       [:= :id id]})]
              (if success
