...
(defn get-all [{:keys [_request _env]}]
  (js-await [{:keys [success results]} (db/query+ {:select    [:p.*
                                                    ...
                                                    :left-join [[:props_teams :t] [:= :p.team_id :t.id]]
                                                    :where     [:= :p.is_active 1]
                                                    :order-by  [[:p.name :asc]]})]
            (if success
...
(defn create [{:keys [request _env]}]
  (js-await [data (cf/request->edn request)]
            (js-await [{:keys [success]} (db/run+ {:insert-into :props_players
...
(defn update [{:keys [route request _env]}]
  (let [id (js/parseInt (-> route :path-params :id) 10)]
    (js-await [data (cf/request->edn request)]
              (js-await [{:keys [success]} (db/run+ {:update :props_players
                                                      :set    {:name          (:name data)
...
(defn delete [{:keys [route _request _env]}]
  (let [id (js/parseInt (-> route :path-params :id) 10)]
    (js-await [{:keys [success]} (db/run+ {:delete-from :props_players
                                            :where       [:= :id id]})]
              (if success
